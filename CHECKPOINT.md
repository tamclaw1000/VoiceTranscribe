...
- Current branch: `main` — the transcript's speaker cell shows each row's raw `Speaker N / Voice M` pair beneath its label, so assigning a name no longer hides the combo, and the Markdown export appends that pair to named rows. Shipped as `v2.4.47` / `90` in one release covering two features (`IMPLEMENTATION.md` #110 and #111): this display change, plus the playback work merged in from `feature/audio-playback-transcript-follow` (`bfa32ab`) — playback of the audio the transcript belongs to with the transcript following the playhead, including for imported files. The two were developed in parallel worktrees that both staged `2.4.46` / `89`, so they were folded together instead of shipping separately: **no release carries `2.4.46`**, and `main` steps from `2.4.45` straight to `2.4.47` so the sequence stays monotonic. Both branches were merged and deleted and both worktrees (`../transcript-speaker-voice`, `../audio-playback`) were removed after the release, so this work now lives on `main` alone.
...
- Worktree layout: the live worktrees are `explore-versions`, `python-version`, `script-paths`, and `avi-audio-import` (kept after its branch merged, on a detached HEAD, holding no uncommitted work). The five already-merged ones (`sidebar-tabs`, `voice-identity-names`, `pause-transcription`, `fix-sentence-occurrence-results`, `settings-publishing`) were moved into `old/`, where git still lists them at their original paths as `prunable`; `old/settings-publishing` is the only copy of the uncommitted `@AppStorage` retraction (`fix/settings-publishing`, `f3d7fcb`).
- `main` state: fast-forward-free merge of `feature/transcript-speaker-voice-display` (`9afc306`), tagged `v2.4.47` and pushed to `origin/main`
- Tooling-only work landed after that release with **no version bump**: the stale `sources/` ignore rule plus the record-unfinished-work convention (`9744681`), and the `extract-audio` script merged from `feature/avi-audio-import`. `main` still reports `2.4.47` / `90` and `v2.4.47` is still the latest tag — the app bundle is unchanged by either.
- **AVI audio import is still not built.** The branch opened for it delivered only the extraction script. What the feature needs is measured and written into `TODO.md` (`# NEW FEATURES`), so none of it has to be re-derived: AVFoundation cannot open the AVI container at all (`isReadable: false`, zero tracks, duration 0, which makes the existing `AVAssetReader` fallback throw), while macOS decodes the same AC-3 bitstream fine once it is wrapped in a container it knows (307,098 frames), and `AVAssetWriter` accepts an AC-3 format description. The work is therefore demux-and-remux, not codec support, and needs no ffmpeg.
...
- Latest tag: `v2.4.47`
- App version on `main`: `2.4.47`, bundle build `90`
- `feature/voice-identity-names` was merged and its local branch deleted; its worktree `../voice-identity-names` is kept on a detached HEAD
- Previous release: `v2.4.44` (`IMPLEMENTATION.md` #108) — split the sidebar into Microphones and AI Selection tabs (worktree `../sidebar-tabs`, kept detached)
...
- Merged branches were pruned back to in-use refs; the remaining worktrees (`explore-versions`, `python-version`, `fix-sentence-occurrence-results`, `pause-transcription`, `sidebar-tabs`, `voice-identity-names`) are intentionally kept
...
- `scripts/extract-audio` - extracts just the audio from any media file into a 16 kHz mono WAV beside the input (`Episode.avi` → `Episode.avi.wav`). Requires ffmpeg. This is how a video container becomes readable by the app's file source, since the app cannot read one itself yet.
