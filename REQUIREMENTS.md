...
- Live Transcript presents timestamp, speaker label when available, and text for each entry, with interim text visually distinct. The transcript row should prioritize speaker identity over repeating the audio source. Show a prominent current-speaker status while diarization is active, and use stable distinct colors to help differentiate speaker labels. Treat every observed `Speaker N / Voice M` pair as a user-editable voice candidate, with `Speaker N / no voice` available for segments that do not have an embedding identity. The collapsible right-hand Voice Identification pane must let the user name a voice candidate and reset it to the generated label without reducing transcript vertical space; renamed voices must update transcript display and exports. Naming a candidate must be possible either by typing or by picking a name already assigned in this session, and picking one must leave the chosen name on that dropdown while hiding the type-in field, until the user switches back to a custom name or resets the row, so a quick reuse cannot be overwritten by accident and only one way to set the name is visible at a time. A "merge same-named speakers" toggle must fold every candidate sharing an assigned name into one speaker — one pane row, one transcript color, and one entry in the exported speaker timeline — while leaving the underlying `Speaker N / Voice M` pairs available for per-row correction. Clicking a transcript row's speaker label must let the user correct the row to any observed voice candidate, cycle candidates, or force a new `Voice N` for that row so under-counted speakers can be represented manually. That menu must list each speaker once — names the user has assigned must not repeat — and a merged speaker's entry must disclose the observed combinations it assigns. Automatic voice identity labels may distinguish same-session voices and are overridden by manual voice names. Associate prompt results beneath the relevant entry and label each by prompt name. Each row must also disclose the `Speaker N / Voice M` pair it belongs to, not only the single collapsed label: assigning a name must not hide which observed combo the row is, and a row that shows one half of a pair must not conceal the other. Raw `Speaker N` / `Voice M` identities, so the disclosure stays stable when names change.
- Live Transcript presents timestamp, speaker label when available, and text for each entry, with interim text visually distinct. The transcript row should prioritize speaker identity over repeating the audio source. Show a prominent current-speaker status while diarization is active, and use stable distinct colors to help differentiate speaker labels. Treat every observed `Speaker N / Voice M` pair as a user-editable voice candidate, with `Speaker N / no voice` available for segments that do not have an embedding identity. The collapsible right-hand Voice Identification pane must let the user name a voice candidate and reset it to the generated label without reducing transcript vertical space; renamed voices must update transcript display and exports. Naming a candidate must be possible either by typing or by picking a name already assigned in this session, and picking one must leave the chosen name on that dropdown while hiding the type-in field, until the user switches back to a custom name or resets the row, so a quick reuse cannot be overwritten by accident and only one way to set the name is visible at a time. A "merge same-named speakers" toggle must fold every candidate sharing an assigned name into one speaker — one pane row, one transcript color, and one entry in the exported speaker timeline — while leaving the underlying `Speaker N / Voice M` pairs available for per-row correction. Clicking a transcript row's speaker label must let the user correct the row to any observed voice candidate, cycle candidates, or force a new `Voice N` for that row so under-counted speakers can be represented manually. That menu must list each speaker once — names the user has assigned must not repeat — and a merged speaker's entry must disclose the observed combinations it assigns. Automatic voice identity labels may distinguish same-session voices and are overridden by manual voice names. Associate prompt results beneath the relevant entry and label each by prompt name.
- **Current voice-identification behavior (supersedes the tuple-name and automatic same-name merge behavior above):** Treat session-local people as distinct from Sortformer speaker slots and WeSpeaker voice clusters. Give each person a stable session ID, generated `Person N` label, editable name, and stable color. In the collapsible right pane, show people and time-stamped diarization audio ranges, including ranges without transcript text; permit naming, explicit merging, selected-range or single-range assignment, creating a person, marking unidentified, restoring the automatic match, undoing an identity edit, and playing a range when its audio is available. Show unresolved manual assignments if a revised diarization range cannot be safely matched. The transcript speaker menu must distinguish correcting the audio range from correcting only that transcript row. Never infer that two same-named people are one person. Do not let a late model result overwrite a manual assignment. Leave short or ambiguous voices unidentified rather than inventing certainty. Preserve raw `Speaker N / Voice M` evidence in the transcript and Markdown export, while using the same person assignment and name in the transcript and speaker timeline.
- Offer playback of the audio the current transcript belongs to: play/pause, a scrubber, and elapsed/total time, shown only when such audio exists. Playback must not start on its own, and a file that cannot be decoded or that has not been finalized must be reported instead of silently failing.
- Dragging the scrubber must move the transcript to the matching location whenever that portion has been transcribed, and must not start playback: scrubbing repositions, it does not play. A position beyond the transcribed content must hold the last transcribed row rather than inventing a location.
- When playing back audio whose timeline the transcript can be mapped onto, the transcript must follow the playhead: scroll the row being spoken into view and mark it, so the reader lands on the passage being played instead of at the newest text, and stop the pane from dragging back to the bottom while following. Clicking a transcript row's timestamp must play the audio from that row. Where no transcript row can be placed in the audio, playback must remain available, and the pane must say that following is unavailable rather than follow an incorrect position or offer a jump that would point at the wrong place.
...
- Retain each entry's position in the source audio when the recognizer reports one, distinct from the wall-clock time the entry was produced. This is what lets an imported file's transcript follow playback and jump to a line, and it is not the same thing as the entry's timestamp.
- While playback of the session's audio is engaged — playing, or parked anywhere after a scrub — the transcript's times must show each entry's position in that audio rather than the session clock time, so they agree with the scrubber and the followed row, and the column must say which of the two it is showing. When playback is at the start, and for audio in which no entry can be placed, clock time remains. An entry with no position in the audio (interim text) keeps clock time rather than showing an invented position.
...
- Support an auto-scroll toggle. Preserve the user's reading position when auto-scroll is off or the user is reviewing earlier content. While the transcript is following playback, the playhead owns the scroll position; auto-scroll resumes governing it when following ends.
- Never present a follow position for audio whose timing is unknown: place an entry by the audio's wall-clock anchor when the app recorded the file in real time, and otherwise by the entry's own position in the source audio, and fall back to clock time when an entry has neither (see section 3.3). Imported audio must be followable on this basis, not merely playable.
...
- The Markdown export must keep the segment's `Speaker N / Voice M` identity even when the segment has an assigned name, so a renamed speaker does not lose the combo the rest of the export identifies it by. This applies to the Markdown document only; the plain-text export and clipboard copy keep the single collapsed label.
