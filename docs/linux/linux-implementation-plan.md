...
- [ ] Choose the initial ASR backend: `faster-whisper`, `whisper.cpp`, or a hosted adapter.
...
- [ ] Create the Linux application directory structure without disturbing the native macOS target.
- [ ] Add a Docker Compose project for the CPU single-container profile.
...
- [ ] Add Linux-specific developer documentation under `docs/linux`.
- [ ] Define environment-variable names and provide a sanitized example file.
- [ ] Define the local persistent-data directory and backup expectations.
...
- [ ] Create the HTTP service using the selected backend framework.
...
- [ ] Add `/health/ready` for model, storage, and queue readiness.
...
- [ ] Add a fake processing mode for development and browser tests.

## 5. Browser client foundation

- [ ] Create the web application shell and routing structure.
- [ ] Add session creation and session selection.
- [ ] Add capability detection and unavailable-feature messaging.
- [ ] Add a responsive source/sidebar layout.
- [ ] Add Live Transcript, Summary, Recent Recordings, and Settings views.
- [ ] Add an accessible notification/error surface.
- [ ] Add reconnecting WebSocket state with visible connection status.
- [ ] Add event sequence tracking and state-snapshot recovery.
...
- [ ] Request microphone permission only when the user starts live capture.
- [ ] Enumerate browser-visible audio devices after permission is granted.
- [ ] Display device name, availability, and selected-device state.
- [ ] Handle permission denied and permission revoked states.
- [ ] Use `AudioWorklet` or an equivalent low-latency capture path.
- [ ] Calculate client-side RMS, peak, and clipping indicators.
...
- [ ] Implement a recording writer independent of transcription and AI jobs.
- [ ] Preserve audio when transcription is slow or unavailable.
- [ ] Store the actual capture format and session start/end timestamps.
...
- [ ] Add FFmpeg, PyAV, or GStreamer to the controlled media-processing image.
- [ ] Normalize ASR input to mono, 16 kHz PCM where required.
- [ ] Preserve the original source when permitted for playback.
- [ ] Track normalization progress and failures.
- [ ] Support the initial chosen format set: WAV, M4A, MP3, FLAC, and other selected formats.
- [ ] Decide whether AVI and video-container extraction are supported directly.
- [ ] Add imported-file metadata and duration display.
- [ ] Add file removal and cleanup behavior.
- [ ] Add tests for malformed, oversized, unsupported, and valid media.

## 9. Live transcription

- [ ] Define a transcription adapter protocol.
- [ ] Implement the selected local ASR adapter.
- [ ] Implement a fake ASR adapter for tests and development.
- [ ] Add model download, cache, version, and readiness handling.
- [ ] Add CPU fallback behavior when the preferred runtime is unavailable.
- [ ] Normalize incoming audio frames for the selected ASR engine.
- [ ] Emit interim transcript events without treating them as final work.
- [ ] Emit finalized segments with audio-relative offsets.
...
- [ ] Add transcription pause/resume while recording continues.
- [ ] Add cancellation and safe final drain behavior.
...
- [ ] Add a file-transcription job endpoint.
- [ ] Run file ASR through an offline adapter where appropriate.
- [ ] Report queued, loading, processing, finalizing, completed, and failed states.
- [ ] Report progress only when a reliable denominator exists.
- [ ] Preserve engine-reported word or segment audio offsets.
...
- [ ] Add authenticated range-capable audio playback.
...
- [ ] Create the minimal application image.
- [ ] Add a persistent `/data` volume.
- [ ] Add SQLite configuration.
- [ ] Add model-cache volume configuration.
- [ ] Verify CPU-only startup without GPU packages.
...
- [ ] Bind the default local deployment to localhost.
...
- [ ] Add structured JSON logs with request/session/job IDs.
...
- [ ] Add Linux setup instructions for CPU Docker Compose.
