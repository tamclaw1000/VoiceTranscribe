...
VoiceTranscribeApp
  └─ AppModel (@MainActor, ObservableObject) — central orchestrator
       ...
       │    └─ SessionIdentityDirectory — people, range assignments, manual edits, undo
...
4. **Speaker diarization — SpeechVAD Sortformer.** `DiarizationCoordinator` uses `SpeechSwiftSortformerDiarizationEngine`, which wraps SpeechVAD's `SortformerStreamingSession`. Audio is downmixed to mono, resampled to 16 kHz, and pushed into Sortformer. Sortformer returns whole-stream speaker time ranges keyed by session-local speaker slots. The app reconciles those ranges into stable session-local range IDs when overlap and start time still describe the same audio. Apple Speech rows are linked by their `audioOffset`; late range updates backfill row labels without delaying text.

5. **Voice identity — SpeechVAD WeSpeaker + app matcher.** `VoiceIdentityService` is always enabled during a transcription session. It buffers the same 16 kHz mono audio used for diarization, extracts WeSpeaker CoreML embeddings from sufficiently long finalized diarization ranges, and matches them against an in-memory `VoiceIdentityMatcher`. Each voice profile keeps several reference embeddings. The matcher accepts a known voice only with sufficient similarity and separation from the runner-up; it creates a new voice only with sufficient duration and novelty. Otherwise the range stays unidentified and may be retried when another profile arrives. These thresholds are provisional, not calibrated biometric guarantees. Nothing is persisted across sessions.

6. **Person assignments and corrections — app layer.** `SessionIdentityDirectory` owns session-local `Person N` IDs, editable names, audio-range assignments, source (`automatic`, `manual`, `unidentified`), a bounded undo history, and explicit merges. Sortformer slots and WeSpeaker clusters remain evidence, not people. Manual range decisions survive later model results. `TranscriptionCoordinator` projects the range's person onto transcript rows by audio offset and preserves explicit row-only overrides; `MarkdownExportService` uses the same projected labels and person IDs. A changed/split range with an orphaned manual assignment is surfaced for review, not silently moved. The right-hand pane lists ranges even when Apple Speech found no words there.
...
10. **Playback and transcript follow — AVFoundation.** `AudioPlaybackService` plays the audio the on-screen transcript belongs to, and `AppModel` maps the playhead onto the transcript through a `TranscriptAudioTarget` anchor (the wall-clock instant equal to audio offset zero) plus the pure `TranscriptPlaybackTimeline`. A recording this app made is placed by an exact anchor, because `RecordingService` opens the file at `RecordingSession.startDate` and writes every captured buffer in real time; an imported file has no such anchor, but is placed by each row's engine-reported position on that file's own timeline, so both follow (see decision 17).

Current limitation: Sortformer slots and WeSpeaker clusters are session-local evidence, not real-world identities. Short or ambiguous audio can remain unidentified. Range reconciliation, similarity thresholds, and the correction UI need validation on real multi-speaker recordings; no identity is persisted between sessions.
...
No CLI flag needed. The `Trace.swift` utility records selected events, including:
- Capture, recording, and transcription button actions (not every UI button)
...
The v2.4.48 person/range correction actions are **not traced**: create, rename, assign ranges, row-only override, mark unidentified, restore automatic, merge, and undo. The log records automatic `voiceIdentity.*` evidence and playback seeks, while transcript/Markdown exports record final labels, but neither records the edit sequence or assignment source. See [the 2026-09-22 telemetry review](docs/dev/voice-identity-telemetry-20260922.md) for observed evidence and export comparison; implementation follow-up is in `TODO.md`. Do not infer a user's correction workflow from automatic voice events alone.
...
| `SessionIdentityDirectory.swift` | Session-local people, diarization range reconciliation, assignments, merge aliases, undo |
...
|---------|-------|-------------|
| 2.4.48 | 91 | Reworked session identity around explicit people and per-range assignments; conservative WeSpeaker abstention, late transcript backfill, scoped correction/merge/undo UI, and person-aligned Markdown timeline. |
| 2.4.47 | 90 | One release carrying two features that were developed in parallel branches and folded together instead of shipping separately (no release carries `2.4.46`). Added playback of the audio the on-screen transcript belongs to, with the transcript following the playhead — scrolling to and highlighting the row being played, and jumping the audio from a clicked timestamp. Following is exact for app-recorded audio (placed by the file's wall-clock anchor) and for imported files (placed by each row's engine-reported position in the file; and the transcript's speaker cell now shows the row's raw `Speaker N / Voice M` pair beneath its label, so naming a row no longer hides the combo it belongs to, with the Markdown export appending that pair to named rows |
