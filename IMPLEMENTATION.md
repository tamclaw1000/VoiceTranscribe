...
## 110. v2.4.47. The Speaker/Voice Pair In The Transcript Display
...
- [x] Bump `CFBundleShortVersionString` to `2.4.47` and `CFBundleVersion` to `90` — one release for this section and section 111 together, see the note below.
- [x] Verify `swift test` passes (94 tests: the playback branch's 91 plus the 3 added here) and `./scripts/build.sh` succeeds, measured on the combined tree rather than on either branch alone.
...
**The numbering collision, resolved.** This section and section 111 were both staged as `2.4.46` / `89` — they were developed in parallel worktrees off the same `v2.4.45` base — and each claimed `#110` in this file. They ship in one release instead: this section keeps `#110` and takes the version, the playback section becomes `#111`, and its subsections become `111a`–`111g`. No release ever carries `2.4.46`, so the sequence on `main` steps from `2.4.45` straight to `2.4.47` and stays monotonic rather than going backwards if the branches had landed in the other order.

## 111. v2.4.47. Audio Playback And Transcript Follow

**Staging note.** This work was developed and verified on `feature/audio-playback-transcript-follow` at `bfa32ab`, where it was staged as `2.4.46` / `89` and numbered `#110`. The display work in section 110 was staged as the same version number in a parallel worktree, so the two were folded into one release at `2.4.47` / `90`, and that branch was merged into this one rather than landed on `main`. The checklist entries below are the record of that staging, so they still name `2.4.46` / `89` where that is what was built and measured at the time.


### 111a. Playing The Transcript's Own Audio
...
### 111b. The Transcript Follows The Playhead
...
### 111c. Where Follow Is Deliberately Off — SUPERSEDED by 111g
...
### 111e. Scrubbing, And Timestamps As Buttons
...
### 111f. The Timestamp Column Reads As Positions During Playback
...
### 111g. Imported Audio Follows After All — The Feature Never Worked On It
...
## 113. v2.4.48. Session Person Identity Rework

- Added `SessionIdentityDirectory`: stable session-local people and per-audio-range assignments, separate from Sortformer speaker slots and WeSpeaker voice clusters. Manual range corrections take precedence over late automatic matches. Revised diarization ranges retain IDs only while they still cover the same audio; orphaned manual assignments remain visible for review.
- WeSpeaker matching now abstains on ambiguous or short evidence instead of forcing an existing or new voice. Profiles retain several reference embeddings, not only a rolling centroid. Match and novelty thresholds are provisional and need calibration on real multi-speaker recordings.
- The right-hand Voice Identification pane lists people and audio ranges, including ranges without transcript text. Users can name and merge people, assign one or selected ranges, explicitly leave a range unidentified, restore the automatic match, undo edits, or play a range when audio is available. The transcript speaker menu offers either an audio-range correction or a row-only override.
- Apple Speech remains the text and sentence-boundary source. Its audio offset links transcript rows to diarization ranges; later speaker evidence backfills rows without replacing a row-only correction. Transcript and Markdown speaker timeline use the same person assignments, while retaining raw `Speaker N / Voice M` evidence.
- Added focused matcher, reconciliation, late-backfill, merge/undo, row-override, and Markdown timeline tests. `swift test`: 102 tests passing. `./scripts/build.sh` completed its clean build and refreshed the signed `dist/VoiceTranscribe.app` at version `2.4.48` / build `91`; warnings were from existing app APIs and external dependencies, not this change.
- Reviewed the first 120-second sample run and `202609222005*` exports. Final person labels were present, but manual edit actions were absent from telemetry; the evidence is in `docs/dev/voice-identity-telemetry-20260922.md`, and the instrumentation/validation work is retained in `TODO.md` rather than blocking this release.
