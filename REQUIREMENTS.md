...
- Live Transcript presents timestamp, speaker label when available, and text for each entry, with interim text visually distinct. The transcript row should prioritize speaker identity over repeating the audio source. Show a prominent current-speaker status while diarization is active, and use stable distinct colors to help differentiate speaker labels. Allow detected speakers to be renamed and reset to their generated `Speaker N` labels; renamed speakers must update transcript display and exports. Associate prompt results beneath the relevant entry and label each by prompt name.
